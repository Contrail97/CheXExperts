...
class GlobalPool(nn.Module):
    ...
    def __init__(self, cfg, cnl):
        ...
        self.expand = 1
        if cfg.model.global_pool == 'AVG_MAX':
            self.expand = 2
        elif cfg.model.global_pool == 'AVG_MAX_LSE':
            self.expand = 3
    ...
    def forward(self, feat_map):
        ...
        if self.cfg.model.global_pool == 'AVG':
            ...
            return self.out_conv(torch.cat((a, b), 1))
        elif self.cfg.model.global_pool == 'AVG_MAX_LSE':
            a = self.avgpool(feat_map)
            b = self.maxpool(feat_map)
            c = self.lse_pool(feat_map)
            return self.out_conv(torch.cat((a, b, c), 1))
