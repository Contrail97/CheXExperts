...
def pso(cfg, dictPseudoBoxesForModel, dictGtBoxes):
    ...
    # Run optimizer
    best_error, best_position = optimizer.optimize(fitFunc, iters=cfg.box_fusion.optimizer_param["max_iterations"], n_processes=2)

    # Print Results
    # print("Best position:", best_position)
    # print("Best error:", best_error)

    return best_error, best_position


def discretePso(cfg, dictPseudoBoxesForModel, dictGtBoxes, target_class):
    import random
    import numpy as np
    ...
    # Define the problem
    objective_function = DiscretePsoFitness(n_model, n_cam, target_class, dictGtBoxes, dictPseudoBoxesForModel, cfg)
    ...
    progressBar = tqdm.tqdm(range(max_iterations), colour='red', file=sys.stdout)
    ...
    return global_best_fitness_value, global_best_position


def binaryPso(cfg, dictPseudoBoxesForModel, dictGtBoxes, target_class):
    ...
    fitFunc = PsoFitness(n_model, n_cam, target_class, dictGtBoxes, dictPseudoBoxesForModel, cfg)
